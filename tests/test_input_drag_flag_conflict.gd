...
func test_quick_right_click_tap() -> bool:
	...
	grid._unhandled_input(press_event)
	...
	grid._unhandled_input(release_event)
...
func test_right_click_drag_suppression() -> bool:
	...
	# 1. Mouse down
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, true, start_pos))
	if grid.get_cell(target_cell_pos).is_flagged:
	...
	grid._unhandled_input(_create_mouse_motion_event(drag_pos, drag_pos - start_pos))

	# 3. Mouse release
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, false, drag_pos))
...
func test_drag_threshold_boundary() -> bool:
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, true, start_pos))
	grid._unhandled_input(_create_mouse_motion_event(jitter_pos, jitter_pos - start_pos))
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, false, jitter_pos))
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, true, start_pos))
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, false, start_pos))
	if grid.get_cell(target_cell_pos).is_flagged:
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, true, start_pos))
	grid._unhandled_input(_create_mouse_motion_event(over_threshold_pos, over_threshold_pos - start_pos))
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, false, over_threshold_pos))
...
func test_middle_click_tap_chord() -> bool:
	...
	# 1. Middle mouse down
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_MIDDLE, true, screen_pos))
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_MIDDLE, false, screen_pos))
...
func test_middle_click_drag_suppression() -> bool:
	...
	# 1. MMB Down
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_MIDDLE, true, start_pos))
	# 2. Motion (Pan)
	grid._unhandled_input(_create_mouse_motion_event(drag_pos, drag_pos - start_pos))
	# 3. MMB Up
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_MIDDLE, false, drag_pos))
...
func test_left_click_immediate_responsiveness() -> bool:
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_LEFT, true, screen_pos))
...
func test_input_state_reset_cleanup() -> bool:
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, true, screen_pos))

	# Reset game
	grid.reset_game()

	# Check that internal state is clean (properties exist and are reset)
	if "_is_right_mouse_down" in grid:
		if grid.get("_is_right_mouse_down") != false:
			print("[FAIL] _is_right_mouse_down not reset to false after reset_game()")
			grid.queue_free()
			return false

	if "_right_mouse_dragged" in grid:
		if grid.get("_right_mouse_dragged") != false:
			print("[FAIL] _right_mouse_dragged not reset to false after reset_game()")
			grid.queue_free()
			return false
