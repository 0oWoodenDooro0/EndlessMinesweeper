...
func test_project_mobile_and_touch_settings() -> bool:
	...
	# 1 = DisplayServer.SCREEN_PORTRAIT (Portrait)
...
func test_touch_quick_tap_reveal() -> bool:
	...
	grid._unhandled_input(touch_down)
	...
	grid._unhandled_input(touch_up)
...
func test_touch_quick_tap_chord_reveal() -> bool:
	...
	grid._unhandled_input(_create_touch_event(0, true, screen_pos))
	...
	grid._unhandled_input(_create_touch_event(0, false, screen_pos))
...
func test_touch_single_finger_drag_pan_and_suppression() -> bool:
	...
	# 1. Touch press
	grid._unhandled_input(_create_touch_event(0, true, start_pos))

	# 2. Touch drag
	grid._unhandled_input(_create_drag_event(0, drag_pos, Vector2(50, 30)))
	...
	# 4. Touch release
	grid._unhandled_input(_create_touch_event(0, false, drag_pos))
...
func test_touch_drag_threshold_jitter_tolerance() -> bool:
	...
	grid._unhandled_input(_create_touch_event(0, true, tap_start_pos))
	grid._unhandled_input(_create_drag_event(0, tap_jitter_pos, Vector2(1.5, 1.5)))
	grid._unhandled_input(_create_touch_event(0, false, tap_jitter_pos))
	...
	grid._unhandled_input(_create_touch_event(0, true, start_pos))
	grid._unhandled_input(_create_drag_event(0, jitter_pos, Vector2(2.0, 2.0)))
	grid._process(0.36)
	grid._unhandled_input(_create_touch_event(0, false, jitter_pos))
...
func test_touch_two_finger_pinch_zoom_and_pan() -> bool:
	...
	grid._unhandled_input(_create_touch_event(0, true, p0_start))
	grid._unhandled_input(_create_touch_event(1, true, p1_start))
	...
	grid._unhandled_input(_create_drag_event(0, p0_pinch, Vector2(-50, 0)))
	grid._unhandled_input(_create_drag_event(1, p1_pinch, Vector2(50, 0)))
	...
	grid._unhandled_input(_create_drag_event(0, p0_in, Vector2(70, 0)))
	grid._unhandled_input(_create_drag_event(1, p1_in, Vector2(-70, 0)))
	...
	grid._unhandled_input(_create_drag_event(0, p0_pan, Vector2(30, 40)))
	grid._unhandled_input(_create_drag_event(1, p1_pan, Vector2(30, 40)))
	...
	# 5. Release touches
	grid._unhandled_input(_create_touch_event(0, false, p0_pan))
	grid._unhandled_input(_create_touch_event(1, false, p1_pan))
...
func test_desktop_compatibility_and_state_reset() -> bool:
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_LEFT, true, Vector2(16, 16)))
	...
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, true, rmb_pos))
	grid._unhandled_input(_create_mouse_button_event(MOUSE_BUTTON_RIGHT, false, rmb_pos))
	...
	grid._unhandled_input(_create_touch_event(0, true, Vector2(50, 50)))
	grid.reset_game()

	# Verify touch states cleaned
	if "_touch_points" in grid and grid.get("_touch_points").size() != 0:
		print("[FAIL] _touch_points not cleared after reset_game()")
		grid.queue_free()
		return false

	if "_is_single_touch_active" in grid and grid.get("_is_single_touch_active") != false:
		print("[FAIL] _is_single_touch_active not reset to false after reset_game()")
		grid.queue_free()
		return false
